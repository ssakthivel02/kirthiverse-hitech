(()=>{
  ...
  function render(){
    ...
    const view=path==='/'?home():path==='/worlds'?worldsPage():path==='/practice'?practicePage():path==='/search'?searchPage():path==='/progress'?progressPage():path==='/mastery'?(window.KV_MASTERY_CONSTELLATION?.markup?.()||progressPage()):path.startsWith('/world/')?worldPage(decodeURIComponent(path.split('/')[2]||'')):path.startsWith('/lesson/')?lessonPage(decodeURIComponent(path.split('/')[2]||'')):home();
  ...
  window.KV_APP_RUNTIME={candidate:CANDIDATE,version:'v25',navigation:'central-delegated',renderLifecycle:'authoritative-app-render',legacyPerLinkHandlers:false,legacyAgeHandlers:false,practiceArena:'v1-local-only',masteryEvidence:'v1-self-reported'};
