(()=>{
  ...
  function setGoal(n){const p=loadPrefs(),pr=loadProgress();p.goal=n;p.baseline=(pr.completed||[]).length;savePrefs(p);render();}
  function togglePref(k){const p=loadPrefs();p[k]=!p[k];savePrefs(p);applyPrefs();render();}
  ...
  function inject(){
    ...
    if(home){const anchor=home.querySelector('.metric-deck')||home.querySelector('.metrics'); if(anchor){anchor.insertAdjacentHTML('afterend',buildPlanner()+buildChallenge()+buildA11y());}}
    else if(document.querySelector('.page')){document.querySelector('.page').insertAdjacentHTML('beforeend',buildA11y())}
    bind();
  }
  function bind(){
    document.querySelectorAll('[data-goal-v8]').forEach(b=>b.onclick=()=>setGoal(Number(b.dataset.goalV8)));
    document.querySelectorAll('[data-pref-v8]').forEach(b=>b.onclick=()=>togglePref(b.dataset.prefV8));
    document.querySelectorAll('[data-v8] [data-link]').forEach(a=>a.addEventListener('click',e=>{e.preventDefault();history.pushState({},'',a.getAttribute('href'));window.dispatchEvent(new PopStateEvent('popstate'));}));
  }
  function render(){applyPrefs();setTimeout(inject,0)}
  new MutationObserver(()=>{clearTimeout(window.__kvv8);window.__kvv8=setTimeout(inject,20)}).observe(document.getElementById('app'),{childList:true,subtree:true});
