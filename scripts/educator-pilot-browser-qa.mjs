...
const cached=await ev(`caches.open('kirthiverse-preview-v46').then(async c=>(await Promise.all(['/educator-pilot-v1.js','/educator-pilot-v1.css'].map(x=>c.match(x).then(Boolean)))).every(Boolean))`);if(!cached)throw Error('educator assets missing from active PWA cache');
