...
assert.equal(manifest.pilot.lastReconciledMain,'97e335d4fcc609c3adee0a0ab99a43aa292767b6');
...
assert.equal(social.status,'CHAPTERS_1_7_IMPLEMENTED_AND_SCHOOL_NEEDS_VALIDATED_CHAPTER8_TEACHING_IMPLEMENTED');
...
assert.equal(social.implementedChapterCount,8);assert.equal(social.schoolNeedsValidatedChapterCount,7);assert.notEqual(social.completionClaim,true);
...
assert.ok(implementedSocial.slice(0,7).every(x=>x.schoolNeedsValidationRequired===true&&x.schoolNeedsValidationPresent===true));
assert.equal(implementedSocial[7].schoolNeedsValidationRequired,true);assert.equal(implementedSocial[7].schoolNeedsValidationPresent,false);
...
assert.equal(socialMap.implementationStatus.implementedChapterCount,8);assert.equal(socialMap.implementationStatus.schoolNeedsValidatedChapterCount,7);assert.notEqual(socialMap.implementationStatus.completionClaim,true);
