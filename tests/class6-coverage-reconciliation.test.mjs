...
const socialRec=[2,3,4,5,6].map(ch=>readJson(`docs/class6-pilot/SOCIAL_SCIENCE_CH${ch}_SOURCE_TOPIC_RECONCILIATION_V1.json`));
...
assert.equal(manifest.pilot.lastReconciledMain,'dec3e2e9479a4fa77bb94b144cc4846e56cd6f51');
...
assert.equal(manifest.priorityDecision.selectedLane,'class6-coverage');assert.equal(manifest.priorityDecision.selectedSubject,'social-science');assert.equal(manifest.priorityDecision.selectedNextChapter,7);assert.equal(manifest.priorityDecision.selectedNextChapterTitle,'India’s Cultural Roots');assert.equal(manifest.priorityDecision.selectedNextTask,'Social Science Chapter 7 Source/Topic Reconciliation');assert.equal(manifest.priorityDecision.requiresFreshConcurrencyCheckBeforeWrite,true);
