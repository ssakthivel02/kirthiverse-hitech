...
assert.equal(maths.schemaVersion,'1.8.0');
...
assert.deepEqual(implementedMath.map(x=>x.chapter),[1,2,3,4,5,6,7]);
assert.equal(pendingMath.length,3);
assert.equal(pendingMath[0].chapter,8);
assert.equal(pendingMath[0].title,'Playing with Constructions');
