...
const mathNeeds=[2,3,4,5,6,7,8].map(ch=>readJson(`docs/class6-pilot/MATH_CH${ch}_SCHOOL_NEEDS_VALIDATION_V1.json`));
...
assert.equal(manifest.schemaVersion,'1.5.0');assert.equal(manifest.pilot.board,'CBSE');assert.equal(manifest.pilot.class,6);assert.equal(manifest.pilot.canonicalRepository,'ssakthivel02/kirthiverse-hitech');assert.equal(manifest.pilot.lastReconciledMain,'0d5089e32747dbb8285f86ed5d9758a5b37ee70f');
...
assert.equal(mathematics.mappedChapterCount,10);assert.equal(mathematics.implementedChapterCount,8);assert.equal(mathematics.remainingMappedChapterCount,2);assert.equal(mathematics.schoolNeedsValidatedChapterCount,8);assert.equal(mathematics.nextLargestGap,'Chapter 9 — Symmetry teaching slice');
for(const chapter of [2,3,4,5,6,7,8])assert.equal(maths[`chapter${chapter}CompletionEvidence`]?.schoolNeedsValidationPresent,true,`Chapter ${chapter} school-needs evidence must be present`);
assert.equal(maths.chapter8CompletionEvidence?.schoolNeedsValidationArtifact,'docs/class6-pilot/MATH_CH8_SCHOOL_NEEDS_VALIDATION_V1.json');assert.equal(maths.chapter8CompletionEvidence?.startupRequestCeilingPreserved,true);assert.equal(maths.chapter8CompletionEvidence?.runtimeCarrier,'data/class6-math-ch2.js');assert.equal(maths.chapter8CompletionEvidence?.sourceOfTruth,'data/class6-math-ch8.js');
...
assert.equal(mathNeeds.at(-1).validationId,'KVS-CBSE6-MATH-CH8-SCHOOL-NEEDS-V1');
assert.equal(manifest.priorityDecision.selectedLane,'mathematics');assert.equal(manifest.priorityDecision.selectedNextChapter,9);assert.equal(manifest.priorityDecision.selectedNextChapterTitle,'Symmetry');assert.equal(manifest.priorityDecision.selectedNextTask,'Chapter 9 Kiki Teaching Slice');assert.equal(manifest.priorityDecision.requiresFreshConcurrencyCheckBeforeWrite,true);
