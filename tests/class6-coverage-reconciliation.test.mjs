...
assert.equal(manifest.schemaVersion,'1.5.0');assert.equal(manifest.pilot.board,'CBSE');assert.equal(manifest.pilot.class,6);assert.equal(manifest.pilot.canonicalRepository,'ssakthivel02/kirthiverse-hitech');assert.equal(manifest.pilot.lastReconciledMain,'8e9fb2401f933da043ac8b629d3ecdd255e937c7');
...
assert.equal(mathematics.mappedChapterCount,10);assert.equal(mathematics.implementedChapterCount,9);assert.equal(mathematics.remainingMappedChapterCount,1);assert.equal(mathematics.schoolNeedsValidatedChapterCount,8);assert.equal(mathematics.nextLargestGap,'Chapter 9 — School Needs Validation');
...
assert.equal(manifest.priorityDecision.selectedLane,'mathematics');assert.equal(manifest.priorityDecision.selectedNextChapter,9);assert.equal(manifest.priorityDecision.selectedNextChapterTitle,'Symmetry');assert.equal(manifest.priorityDecision.selectedNextTask,'Chapter 9 School Needs Validation');assert.equal(manifest.priorityDecision.requiresFreshConcurrencyCheckBeforeWrite,true);
