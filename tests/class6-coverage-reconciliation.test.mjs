...
const socialNeeds=[1,2,3,4,5,6,7,8].map(ch=>readJson(`docs/class6-pilot/SOCIAL_SCIENCE_CH${ch}_SCHOOL_NEEDS_VALIDATION_V1.json`));
...
assert.equal(manifest.pilot.lastReconciledMain,'03f3202dd0fb6224244ae9ac05e6572e61de36e4');
...
assert.equal(social.status,'CHAPTERS_1_8_IMPLEMENTED_AND_SCHOOL_NEEDS_VALIDATED');
...
assert.equal(social.implementedChapterCount,8);assert.equal(social.schoolNeedsValidatedChapterCount,8);assert.notEqual(social.completionClaim,true);
for(const ch of [2,3,4,5,6,7,8]){assert.equal(social[`chapter${ch}SchoolNeedsValidationRequired`],true);assert.equal(social[`chapter${ch}SchoolNeedsValidationPresent`],true);assert.equal(social[`chapter${ch}SchoolNeedsValidationArtifact`],`docs/class6-pilot/SOCIAL_SCIENCE_CH${ch}_SCHOOL_NEEDS_VALIDATION_V1.json`);}
...
assert.equal(social.chapter8SchoolNeedsValidationRequired,true);assert.equal(social.chapter8SchoolNeedsValidationPresent,true);assert.equal(social.chapter8SchoolNeedsValidationArtifact,'docs/class6-pilot/SOCIAL_SCIENCE_CH8_SCHOOL_NEEDS_VALIDATION_V1.json');
...
const implementedSocial=socialMap.chapters.slice(0,8);assert.ok(implementedSocial.every(x=>x.status==='KIKI_TEACHING_SLICE_COMPLETE'));
assert.deepEqual(implementedSocial.map(x=>x.topicCount),[3,4,4,4,4,4,4,4]);assert.deepEqual(implementedSocial.map(x=>x.assessmentCount),[15,20,20,20,20,20,20,20]);
...
const ch8=socialMap.chapters[7];assert.equal(ch8.title,'Unity in Diversity, or ‘Many in the One’');assert.equal(ch8.status,'KIKI_TEACHING_SLICE_COMPLETE');assert.equal(ch8.topicCount,4);assert.equal(ch8.lessonDataset,'data/class6-social-science-ch8.js');assert.equal(ch8.assessmentDataset,'data/class6-social-science-ch8-assessments.js');assert.equal(ch8.assessmentCount,20);assert.equal(ch8.sourceTopicReconciliationArtifact,'docs/class6-pilot/SOCIAL_SCIENCE_CH8_SOURCE_TOPIC_RECONCILIATION_V1.json');assert.equal(ch8.schoolNeedsValidationRequired,true);assert.equal(ch8.schoolNeedsValidationPresent,true);assert.equal(ch8.schoolNeedsValidationArtifact,'docs/class6-pilot/SOCIAL_SCIENCE_CH8_SCHOOL_NEEDS_VALIDATION_V1.json');
assert.ok(socialMap.chapters.slice(8).every(x=>x.status==='MAPPED_NOT_YET_IMPLEMENTED'));
assert.equal(socialMap.implementationStatus.implementedChapterCount,8);assert.equal(socialMap.implementationStatus.schoolNeedsValidatedChapterCount,8);assert.notEqual(socialMap.implementationStatus.completionClaim,true);
...
assert.equal(socialNeeds[7].validationId,'KVS-CBSE6-SOC-CH8-SCHOOL-NEEDS-V1');assert.ok(socialNeeds[7].scenarios.every(x=>x.safeLearnerAction.length>80&&x.masteryCriterion.length>80));assert.equal(socialNeeds.flatMap(x=>x.scenarios).length,64);
const ch8NeedsText=JSON.stringify(socialNeeds[7]).toLowerCase();for(const required of ['diversity','uniformity','food','textile','festival','story','adaptation','authenticity','offline','devotional','political'])assert.ok(ch8NeedsText.includes(required),`missing Chapter 8 school-needs boundary ${required}`);
...
assert.equal(manifest.priorityDecision.selectedLane,'class6-coverage');assert.equal(manifest.priorityDecision.selectedSubject,'social-science');assert.equal(manifest.priorityDecision.selectedNextChapter,9);assert.equal(manifest.priorityDecision.selectedNextChapterTitle,'Family and Community');assert.equal(manifest.priorityDecision.selectedNextTask,'Social Science Chapter 9 Source/Topic Reconciliation');assert.equal(manifest.priorityDecision.requiresFreshConcurrencyCheckBeforeWrite,true);
