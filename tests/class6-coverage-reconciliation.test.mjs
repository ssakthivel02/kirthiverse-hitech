...
assert.equal(manifest.schemaVersion,'1.20.0');
...
assert.equal(manifest.pilot.lastReconciledMain,'e25cd06ca01c13ff97bb3dc297e742a88f3bf210');
...
assert.equal(social.status,'CHAPTERS_1_2_3_4_5_6_7_IMPLEMENTED_CHAPTERS_1_2_3_4_5_6_SCHOOL_NEEDS_VALIDATED');
...
assert.equal(social.implementedChapterCount,7);assert.equal(social.schoolNeedsValidatedChapterCount,6);assert.notEqual(social.completionClaim,true);
...
assert.equal(social.chapter7SchoolNeedsValidationRequired,true);assert.equal(social.chapter7SchoolNeedsValidationPresent,false);assert.equal(social.chapter7SchoolNeedsValidationArtifact,undefined);
...
assert.ok(implementedSocial.slice(0,6).every(x=>x.schoolNeedsValidationRequired===true&&x.schoolNeedsValidationPresent===true));
...
const ch7=socialMap.chapters[6];assert.equal(ch7.title,'India’s Cultural Roots');assert.equal(ch7.status,'KIKI_TEACHING_SLICE_COMPLETE');assert.equal(ch7.topicCount,4);assert.equal(ch7.lessonDataset,'data/class6-social-science-ch7.js');assert.equal(ch7.assessmentDataset,'data/class6-social-science-ch7-assessments.js');assert.equal(ch7.assessmentCount,20);assert.equal(ch7.sourceTopicReconciliationArtifact,'docs/class6-pilot/SOCIAL_SCIENCE_CH7_SOURCE_TOPIC_RECONCILIATION_V1.json');assert.equal(ch7.schoolNeedsValidationRequired,true);assert.equal(ch7.schoolNeedsValidationPresent,false);assert.equal(ch7.schoolNeedsValidationArtifact,undefined);
assert.ok(socialMap.chapters.slice(7).every(x=>x.status==='MAPPED_NOT_YET_IMPLEMENTED'));
assert.equal(socialMap.implementationStatus.implementedChapterCount,7);assert.equal(socialMap.implementationStatus.schoolNeedsValidatedChapterCount,6);assert.notEqual(socialMap.implementationStatus.completionClaim,true);
...
assert.equal(manifest.priorityDecision.selectedLane,'class6-coverage');assert.equal(manifest.priorityDecision.selectedSubject,'social-science');assert.equal(manifest.priorityDecision.selectedNextChapter,7);assert.equal(manifest.priorityDecision.selectedNextChapterTitle,'India’s Cultural Roots');assert.equal(manifest.priorityDecision.selectedNextTask,'Social Science Chapter 7 School-Needs Validation');assert.equal(manifest.priorityDecision.requiresFreshConcurrencyCheckBeforeWrite,true);
