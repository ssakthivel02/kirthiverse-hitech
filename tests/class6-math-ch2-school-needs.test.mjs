...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/MATH_CH2_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-MATH-CH2-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 2 · Lines and Angles/);
...
const implementedTopicIds=new Set(map.chapter2Topics.filter(x=>x.status==='END_TO_END_PILOT_IMPLEMENTED').map(x=>x.id));
...
for(const scenario of validation.scenarios){
  ...
  assert.equal(linked.length,5,`${scenario.id} requires exactly five linked Chapter 2 assessments`);
...
for(const topic of map.chapter2Topics){
  assert.ok((topicCoverage.get(topic.id)||0)>=2,`${topic.id} needs at least two school-needs scenarios`);
}
assert.ok((topicCoverage.get('TOPIC-MATH6-02-03')||0)>=3,'measurement/construction topic should cover alignment, scale and bisector evidence needs');

const allText=JSON.stringify(validation).toLowerCase();
for(const required of ['endpoint','right angle','arm length','protractor','correct zero','offline','no network-dependent tutoring step','bisector']){
  assert.ok(allText.includes(required),`missing Chapter 2 school-needs boundary: ${required}`);
}
...
console.log(`CLASS6_MATH_CH2_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
