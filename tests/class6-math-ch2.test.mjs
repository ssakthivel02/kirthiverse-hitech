...
for(const lesson of lessons){
  assert.equal(lesson.subject,'Mathematics');assert.equal(lesson.board,'CBSE');assert.equal(lesson.classLevel,6);assert.equal(lesson.book,'Ganita Prakash');assert.equal(lesson.chapter,2);assert.equal(lesson.chapterTitle,'Lines and Angles');assert.equal(lesson.ageBand,'11-13');
...
for(const lessonId of lessonIds){const attached=assessments.filter(x=>x.lessonId===lessonId);assert.equal(attached.length,5,`${lessonId} assessment count`);for(const item of attached)for(const field of ['stableAssessmentId','assessmentType','questionActivity','correctAnswer','hint','explanation'])assert.ok(item[field],`assessment missing ${field}`);assert.ok(attached.some(x=>x.assessmentType==='reasoning'));assert.ok(attached.some(x=>x.assessmentType==='mastery_check'))}
assert.ok(assessments.some(x=>/Ray AB starts at A/i.test(x.correctAnswer)));
assert.ok(assessments.some(x=>/arm length/i.test(x.questionActivity)||/arm length/i.test(x.explanation)));
assert.ok(assessments.some(x=>/opposite scale|correct scale|scale/i.test(x.questionActivity)&&/125°/.test(x.correctAnswer)));
assert.ok(assessments.some(x=>/360°/.test(x.correctAnswer)&&/230°/.test(x.correctAnswer)));
assert.ok(assessments.some(x=>/42°/.test(x.correctAnswer)&&/bisector/i.test(x.explanation)));
...
assert.match(entry,/class6MathDeferredAssessments/);assert.ok(entry.includes('math\\.cbse6\\.ganita-prakash'));assert.ok(entry.includes('patterns\\.')&&entry.includes('ch(?:2|3|4|5|6|7)\\.'),'Chapter 1-7 deferred-assessment route coverage missing');assert.ok(!entry.includes("/^\\/lesson\\/math\\./"));assert.match(entry,/release-closure/);
const baseChunk=entry.slice(entry.indexOf('const mathBaseFiles'),entry.indexOf('const loadClass6MathPilot'));assert.ok(baseChunk.includes('data/class6-math-ch2.js'));assert.ok(baseChunk.includes('deferredMathAssessments'));assert.ok(!baseChunk.includes("['class6-math-ch1-assessments','data/class6-math-assessments.js']"));
const sw=fs.readFileSync('sw-v30.js','utf8');assert.match(sw,/kirthiverse-preview-v45/);assert.match(sw,/MANUS-VISUAL-MASTER-05-PWA-45/);for(const asset of ['/data/class6-math-ch2.js','/data/class6-math-ch2-assessments.js']) assert.ok(sw.includes(asset));
