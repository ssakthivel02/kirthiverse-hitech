...
assert.equal(runtimeSandbox.window.KV_LESSONS.length,12,'consolidated Chapters 3-6 runtime bundle must expose twelve lessons');
assert.ok(lessonIds.every(id=>runtimeSandbox.window.KV_LESSONS.some(x=>x.id===id)),'runtime bundle missing Chapter 4 lesson');
assert.equal(new Set(runtimeSandbox.window.KV_LESSONS.map(x=>x.id)).size,12,'runtime bundle lesson IDs must remain unique');
...
assert.ok(baseChunk.includes('data/class6-math-ch3-4-runtime.js'),'combined Chapter 3-6 lesson runtime must load in base slice');
