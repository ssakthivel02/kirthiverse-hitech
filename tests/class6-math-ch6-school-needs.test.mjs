...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/MATH_CH6_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-MATH-CH6-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 6 · Perimeter and Area/);
...
for(const scenario of validation.scenarios){
  ...
  assert.equal(linked.length,5,`${scenario.id} requires exactly five linked Chapter 6 assessments`);
...
for(const topic of map.chapter6Topics){
  assert.ok((topicCoverage.get(topic.id)||0)>=2,`${topic.id} needs at least two school-needs scenarios`);
}
assert.ok((topicCoverage.get('TOPIC-MATH6-06-02')||0)>=3,'area topic should cover method/units, grid counting and equal-area/different-perimeter needs');
...
for(const required of ['perimeter','outer boundary','linear unit','area','square unit','grid lines','equal-area','perpendicular','right angle','one-half','no gap or overlap','offline','network-dependent tutoring']){
...
console.log(`CLASS6_MATH_CH6_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
