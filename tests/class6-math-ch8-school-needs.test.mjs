...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/MATH_CH8_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-MATH-CH8-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 8 · Playing with Constructions/);
...
for(const scenario of validation.scenarios){
  ...
  assert.equal(linked.length,5,`${scenario.id} requires exactly five linked Chapter 8 assessments`);
...
for(const topic of map.chapter8Topics) assert.ok((topicCoverage.get(topic.id)||0)>=2,`${topic.id} needs at least two school-needs scenarios`);
assert.ok((topicCoverage.get('TOPIC-MATH6-08-01')||0)>=3,'fixed-distance topic should cover compass-width preservation, centre/radius meaning and intersecting-arc verification');
...
console.log(`CLASS6_MATH_CH8_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
