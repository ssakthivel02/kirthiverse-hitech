...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/MATH_CH9_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-MATH-CH9-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 9 · Symmetry/);
...
const implementedTopics=map.chapter9Topics.filter(x=>x.status==='END_TO_END_PILOT_IMPLEMENTED');
const implementedTopicIds=new Set(implementedTopics.map(x=>x.id));
const topicById=new Map(implementedTopics.map(x=>[x.id,x]));
...
for(const scenario of validation.scenarios){
  ...
  const topic=topicById.get(scenario.topicId);
  assert.ok(topic.misconceptions.includes(scenario.expectedMisconception),`${scenario.id} expected misconception is not in the mapped Chapter 9 topic contract`);
  ...
  assert.equal(linked.length,5,`${scenario.id} requires exactly five linked Chapter 9 assessments`);
...
for(const topic of map.chapter9Topics) assert.ok((topicCoverage.get(topic.id)||0)>=2,`${topic.id} needs at least two school-needs scenarios`);
assert.ok((topicCoverage.get('TOPIC-MATH6-09-01')||0)>=3,'reflection topic should cover exact correspondence, reflected distance and candidate-axis/orientation reasoning');
assert.ok((topicCoverage.get('TOPIC-MATH6-09-03')||0)>=3,'rotation topic should cover non-trivial rotation, order/angle distinction and offline completion');

const allText=JSON.stringify(validation).toLowerCase();
for(const required of ['reflection','perpendicular distance','mirror line','multiple','two-axis','circle','360 degrees','rotational order','smallest positive angle','mirror-line count','offline','network-dependent tutoring']){
  assert.ok(allText.includes(required),`missing Chapter 9 school-needs boundary: ${required}`);
}
...
console.log(`CLASS6_MATH_CH9_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
