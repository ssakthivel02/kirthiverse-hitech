...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/MATH_CH1_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
for(const scenario of validation.scenarios){
  ...
  assert.ok(scenario.safeLearnerAction?.length>30,`${scenario.id} safe learner action too weak`);
  assert.ok(scenario.masteryCriterion?.length>30,`${scenario.id} mastery criterion too weak`);
...
assert.match(allText,/offline/);
assert.match(allText,/no network-dependent tutoring step/);
assert.match(allText,/without needing a photo or personal information/);

console.log(`CLASS6_MATH_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
