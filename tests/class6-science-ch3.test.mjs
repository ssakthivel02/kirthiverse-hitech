...
assert.equal(map.chapters[2].title,'Mindful Eating: A Path to a Healthy Body');
...
assert.ok(map.chapter3Topics.every(x=>Array.isArray(x.competencies)&&x.competencies.length>=3));
assert.ok(map.chapter3Topics.every(x=>Array.isArray(x.misconceptions)&&x.misconceptions.length>=3));
assert.ok(map.chapter3Topics.every(x=>Array.isArray(x.safetyBoundary)&&x.safetyBoundary.length>=3));
assert.equal(map.chapter3CompletionEvidence.lessonCount,3);
assert.equal(map.chapter3CompletionEvidence.assessmentCount,15);
assert.equal(map.chapter3CompletionEvidence.childNutritionSafetyRequired,true);
assert.equal(map.chapter3CompletionEvidence.status,'CHAPTER3_KIKI_TEACHING_SLICE_COMPLETE');
assert.match(map.chapter3CompletionRule,/dieting|weight/i);
assert.match(map.chapter3CompletionRule,/allergy self-testing/i);
...
for(const lesson of lessons){
  ...
  assert.ok(lesson.content.length>600);
...
assert.deepEqual(assessments.map(x=>x.stableAssessmentId),Array.from({length:15},(_,i)=>`KV-CBSE6-SCI-${String(i+31).padStart(4,'0')}`));
...
for(const required of ['food diversity','hydration','hygiene','marketing claim','trusted adult','qualified healthcare']) assert.ok(allText.includes(required),`missing Chapter 3 safety/learning concept: ${required}`);
assert.ok(allText.includes('no calorie')||allText.includes('without calorie')||allText.includes('calorie target'));
assert.ok(allText.includes('weight-loss')||allText.includes('weight control'));
assert.ok(allText.includes('allergy self-testing')||allText.includes('should not test suspected allergies'));
assert.ok(allText.includes('supplement')&&allText.includes('treatment'));
assert.ok(!/\b(?:800|1000|1200|1500|1800|2000)\s*(?:kcal|calories)\b/i.test(allText),'numeric child calorie prescription detected');
assert.ok(!/\bbmi\s*(?:target|goal|score)\b/i.test(allText),'BMI targeting detected');

const entry=fs.readFileSync('p0-entry-v1.js','utf8');
for(const asset of ['data/class6-science-ch3.js','data/class6-science-ch3-assessments.js']) assert.ok(entry.includes(asset),`loader missing ${asset}`);
assert.match(entry,/datasetKey:'class6SciencePilot'/);
assert.match(entry,/CBSE6-SCI-CH3-1/);
const sw=fs.readFileSync('sw-v30.js','utf8');
assert.match(sw,/kirthiverse-preview-v36/);
for(const asset of ['/data/class6-science-ch3.js','/data/class6-science-ch3-assessments.js']) assert.ok(sw.includes(asset),`precache missing ${asset}`);
...
console.log(`CLASS6_SCIENCE_CH3_PASS topics=${map.chapter3Topics.length} lessons=${lessons.length} assessments=${assessments.length} childNutritionSafety=${map.chapter3CompletionEvidence.childNutritionSafetyRequired}`);
