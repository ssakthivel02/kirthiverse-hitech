...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SCIENCE_CH8_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
const implementedTopicIds=new Set(map.chapter8Topics.filter(x=>x.status==='END_TO_END_PILOT_IMPLEMENTED').map(x=>x.id));
...
for(const topic of map.chapter8Topics) assert.ok((coverage.get(topic.id)||0)>=2,`${topic.id} needs at least two school-needs scenarios`);
const allText=JSON.stringify(validation).toLowerCase();
for(const required of ['water vapour','visible','evaporation','boiling','condensation','sealed','kettle','steam','fair comparison','weather forecast','offline']) assert.ok(allText.includes(required),`missing Chapter 8 readiness boundary ${required}`);
for(const forbidden of ['real child data collected','remote teacher monitoring enabled','child-facing cloud ai enabled','school-specific pacing verified','boil the water yourself','put your hand in steam','drink the test water','heat a sealed container','climb onto the roof']) assert.ok(!allText.includes(forbidden),`forbidden readiness claim/instruction found: ${forbidden}`);
assert.match(allText,/reject the hazardous plan/);
assert.match(allText,/cannot by itself provide a weather forecast/);
assert.match(allText,/without sensors, uploads, ingestion, climbing or heat sources/);
console.log(`CLASS6_SCIENCE_CH8_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${coverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
