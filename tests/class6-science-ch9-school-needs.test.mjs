...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SCIENCE_CH9_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.match(validation.claimBoundary,/not .*laboratory safety certification|laboratory safety certification/i);
assert.match(validation.claimBoundary,/drinking-water safety assessment/i);
...
const implementedTopicIds=new Set(map.chapter9Topics.filter(x=>x.status==='END_TO_END_PILOT_IMPLEMENTED').map(x=>x.id));
...
for(const topic of map.chapter9Topics) assert.ok((coverage.get(topic.id)||0)>=2,`${topic.id} needs at least two school-needs scenarios`);
const allText=JSON.stringify(validation).toLowerCase();
for(const required of ['property difference','filtration','dissolved','potability','clear','winnowing','dust','powered','kettle','hot plate','evaporation','multi-step','chemically pure','offline']) assert.ok(allText.includes(required),`missing Chapter 9 readiness boundary ${required}`);
for(const forbidden of ['real child data collected','remote teacher monitoring enabled','child-facing cloud ai enabled','school-specific pacing verified','taste the unknown mixture','drink the filtered water','inhale the dust','boil the solution yourself','heat a sealed container']) assert.ok(!allText.includes(forbidden),`forbidden readiness claim/instruction found: ${forbidden}`);
assert.match(allText,/reject the dusty powered demonstration/);
assert.match(allText,/reject the hazardous heating plan/);
assert.match(allText,/does not by itself establish complete chemical purity/);
assert.match(allText,/without sensors, uploads, ingestion, unknown substances, dust generation, powered devices or heat sources/);
console.log(`CLASS6_SCIENCE_CH9_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${coverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
