...
const allAssessmentText=JSON.stringify(assessments).toLowerCase();
assert.match(allAssessmentText,/evidence/);
assert.match(allAssessmentText,/prediction/);
assert.match(allAssessmentText,/without collecting personal data|teacher-provided anonymous/);

const entry=fs.readFileSync('p0-entry-v1.js','utf8');
for(const asset of ['data/class6-science-pilot.js','data/class6-science-assessments.js']) assert.ok(entry.includes(asset),`loader missing ${asset}`);
