...
const entry=fs.readFileSync('p0-entry-v1.js','utf8');for(const asset of ['data/class6-science-pilot.js','data/class6-science-assessments.js'])assert.ok(entry.includes(asset));assert.match(entry,/class6SciencePilot/);assert.match(entry,/load-error/);assert.match(entry,/CBSE6-SCI-CH12-1/);
const sw=fs.readFileSync('sw-v30.js','utf8');assert.match(sw,/kirthiverse-preview-v40/);for(const asset of ['/data/class6-science-pilot.js','/data/class6-science-assessments.js'])assert.ok(sw.includes(asset));
