...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH1_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH1-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 1 · Locating Places on the Earth/);
assert.match(validation.claimBoundary,/not a live student study/i);
assert.match(validation.claimBoundary,/school-specific pacing validation/i);
...
vm.runInContext(fs.readFileSync('data/class6-social-science-pilot.js','utf8'),sandbox,{filename:'class6-social-science-pilot.js'});
vm.runInContext(fs.readFileSync('data/class6-social-science-assessments.js','utf8'),sandbox,{filename:'class6-social-science-assessments.js'});
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===1);
const lessonIds=new Set(lessons.map(x=>x.id));
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessonIds.has(x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===1);
assert.ok(chapter);
...
for(const scenario of validation.scenarios){
  ...
  assert.ok(scenario.safeLearnerAction?.length>50,`${scenario.id} safe learner action too weak`);
  assert.ok(scenario.masteryCriterion?.length>50,`${scenario.id} mastery criterion too weak`);
...
for(const topicId of validTopicIds) assert.ok((topicCoverage.get(topicId)||0)>=2,`${topicId} needs at least two school-needs scenarios`);

const allText=JSON.stringify(validation).toLowerCase();
for(const required of ['scale','direction','legend','latitude','longitude','equator','prime meridian','local solar time','standard time','offline','precise-location','network-dependent tutoring']) assert.ok(allText.includes(required),`missing Social Science school-needs boundary: ${required}`);
for(const forbidden of ['real child data collected','remote teacher monitoring enabled','child-facing cloud ai enabled','school-specific pacing verified','live device location required','home coordinates required','photo upload required','camera required']) assert.ok(!allText.includes(forbidden),`forbidden claim found: ${forbidden}`);

console.log(`CLASS6_SOCIAL_SCIENCE_CH1_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
