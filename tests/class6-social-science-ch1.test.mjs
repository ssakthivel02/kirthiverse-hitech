...
assert.equal(map.schemaVersion,'1.1.0');
assert.equal(ch1.status,'KIKI_TEACHING_SLICE_COMPLETE');
assert.equal(ch1.topicCount,3);assert.equal(ch1.assessmentCount,15);
assert.equal(ch1.schoolNeedsValidationRequired,true);assert.equal(ch1.schoolNeedsValidationPresent,false);
assert.equal(map.implementationStatus.implementedChapterCount,1);
assert.equal(map.implementationStatus.schoolNeedsValidatedChapterCount,0);
