...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH2_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH2-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 2 · Oceans and Continents/);
...
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===2);
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessons.some(l=>l.id===x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===2);
assert.ok(chapter);
...
for(const required of ['hemisphere','five named oceans','interconnection','continent','island','convention','ocean-impact','offline']) assert.ok(allText.includes(required),`missing Chapter 2 school-needs boundary: ${required}`);
...
console.log(`CLASS6_SOCIAL_SCIENCE_CH2_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
