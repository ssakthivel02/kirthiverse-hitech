...
const ch2=map.chapters.find(x=>x.chapter===2);assert.equal(map.schemaVersion,'1.4.0');assert.equal(ch2.status,'KIKI_TEACHING_SLICE_COMPLETE');assert.equal(ch2.topicCount,4);assert.equal(ch2.assessmentCount,20);assert.equal(ch2.schoolNeedsValidationRequired,true);assert.equal(ch2.schoolNeedsValidationPresent,false);assert.equal(map.implementationStatus.implementedChapterCount,2);assert.equal(map.implementationStatus.schoolNeedsValidatedChapterCount,1);assert.equal(map.implementationStatus.completionClaim,false);
...
const text=JSON.stringify({lessons:ch2Lessons,assessments:ch2Assessments}).toLowerCase();for(const forbidden of ['share your home address','upload your precise location','share your precise coordinates','live gps','plate tectonics','bathymetry','marine chemistry'])assert.ok(!text.includes(forbidden),`forbidden Chapter 2 boundary found: ${forbidden}`);
