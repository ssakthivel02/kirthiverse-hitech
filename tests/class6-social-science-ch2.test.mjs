...
const ch2=map.chapters.find(x=>x.chapter===2);assert.equal(map.schemaVersion,'1.11.0');assert.equal(ch2.status,'KIKI_TEACHING_SLICE_COMPLETE');assert.equal(ch2.topicCount,4);assert.equal(ch2.assessmentCount,20);assert.equal(ch2.schoolNeedsValidationRequired,true);assert.equal(ch2.schoolNeedsValidationPresent,true);assert.equal(map.implementationStatus.implementedChapterCount,4);assert.equal(map.implementationStatus.schoolNeedsValidatedChapterCount,4);assert.equal(map.implementationStatus.completionClaim,false);
