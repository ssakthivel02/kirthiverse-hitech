...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH3_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH3-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 3 · Landforms and Life/);
...
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===3);
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessons.some(l=>l.id===x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===3);
assert.ok(chapter);
...
for(const required of ['major landforms','mixed landscape','possibilities','mountain','plateau','resource','plain','flood','offline']) assert.ok(allText.includes(required),`missing Chapter 3 school-needs boundary: ${required}`);
...
console.log(`CLASS6_SOCIAL_SCIENCE_CH3_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
