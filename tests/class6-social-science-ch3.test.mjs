...
for(const stereotype of ['mountain people always','plateau people always','plain people always','all plateaus are mineral-rich','all plains are fertile','everyone in mountains must'])assert.ok(!text.includes(stereotype),`deterministic human-geography stereotype found: ${stereotype}`);
assert.ok(text.includes('depends on')||text.includes('can')||text.includes('may'),'Chapter 3 should use conditional geographic reasoning');
