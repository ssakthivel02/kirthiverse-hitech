...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH4_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH4-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 4 · Timeline and Sources of History/);
...
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===4);
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessons.some(l=>l.id===x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===4);
assert.ok(chapter);
...
for(const required of ['evidence','inference','uncertainty','bce','century','source','corroborat','early communities','offline']) assert.ok(allText.includes(required),`missing Chapter 4 school-needs boundary: ${required}`);
for(const forbidden of ['real child data collected','remote teacher monitoring enabled','child-facing cloud ai enabled','school-specific pacing verified','home address required','photo upload required','camera required','religion disclosure required": true','caste disclosure required": true','ancestry disclosure required": true','genetics disclosure required": true']) assert.ok(!allText.includes(forbidden),`forbidden claim found: ${forbidden}`);

console.log(`CLASS6_SOCIAL_SCIENCE_CH4_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
