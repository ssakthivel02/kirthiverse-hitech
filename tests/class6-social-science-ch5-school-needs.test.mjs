...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH5_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH5-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 5 · India, That Is Bharat/);
...
vm.runInContext(fs.readFileSync('data/class6-social-science-ch5.js','utf8'),sandbox,{filename:'class6-social-science-ch5.js'});
vm.runInContext(fs.readFileSync('data/class6-social-science-ch5-assessments.js','utf8'),sandbox,{filename:'class6-social-science-ch5-assessments.js'});
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===5);
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessons.some(l=>l.id===x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===5);
assert.ok(chapter);
...
for(const required of ['geographical','historical','source','scope','sapta sindhava','jambudvipa','bharata','sindhu','cross-cultural','constitutional','political','offline']) assert.ok(allText.includes(required),`missing Chapter 5 school-needs boundary: ${required}`);
for(const forbidden of ['real child data collected','remote teacher monitoring enabled','child-facing cloud ai enabled','school-specific pacing verified','home address required','religion disclosure required": true','caste disclosure required": true','ancestry disclosure required": true','political opinion disclosure required": true','preferred country name disclosure required": true','patriotic loyalty required']) assert.ok(!allText.includes(forbidden),`forbidden claim found: ${forbidden}`);

console.log(`CLASS6_SOCIAL_SCIENCE_CH5_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
