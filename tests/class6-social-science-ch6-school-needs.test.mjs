...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH6_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH6-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 6 · The Beginnings of Indian Civilisation/);
...
vm.runInContext(fs.readFileSync('data/class6-social-science-ch6.js','utf8'),sandbox,{filename:'class6-social-science-ch6.js'});
vm.runInContext(fs.readFileSync('data/class6-social-science-ch6-assessments.js','utf8'),sandbox,{filename:'class6-social-science-ch6-assessments.js'});
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===6);
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessons.some(l=>l.id===x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===6);
assert.ok(chapter);
...
for(const required of ['civilisation','archaeolog','evidence','uncertainty','harappan','settlement','drain','water','craft','exchange','undeciphered','multiple','offline']) assert.ok(allText.includes(required),`missing Chapter 6 school-needs boundary: ${required}`);
for(const forbidden of ['real child data collected','remote teacher monitoring enabled','child-facing cloud ai enabled','school-specific pacing verified','home address required','religion disclosure required": true','caste disclosure required": true','ancestry disclosure required": true','political opinion disclosure required": true','harappan script deciphered": true','civilisational superiority required','single cause proven']) assert.ok(!allText.includes(forbidden),`forbidden claim found: ${forbidden}`);

console.log(`CLASS6_SOCIAL_SCIENCE_CH6_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
