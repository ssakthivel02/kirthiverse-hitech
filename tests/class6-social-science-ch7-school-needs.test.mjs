...
const validation=JSON.parse(fs.readFileSync('docs/class6-pilot/SOCIAL_SCIENCE_CH7_SCHOOL_NEEDS_VALIDATION_V1.json','utf8'));
...
assert.equal(validation.validationId,'KVS-CBSE6-SOC-CH7-SCHOOL-NEEDS-V1');
assert.equal(validation.validationType,'SYNTHETIC_SCHOOL_LEARNING_NEEDS_READINESS');
assert.match(validation.scope,/Chapter 7 · India’s Cultural Roots/);
...
vm.runInContext(fs.readFileSync('data/class6-social-science-ch7.js','utf8'),sandbox,{filename:'class6-social-science-ch7.js'});
vm.runInContext(fs.readFileSync('data/class6-social-science-ch7-assessments.js','utf8'),sandbox,{filename:'class6-social-science-ch7-assessments.js'});
const lessons=sandbox.window.KV_LESSONS.filter(x=>x.chapter===7);
const assessments=sandbox.window.KV_ASSESSMENTS.filter(x=>lessons.some(l=>l.id===x.lessonId));
const chapter=map.chapters.find(x=>x.chapter===7);
assert.ok(chapter);
...
for(const required of ['vedas','oral','source','belief neutrality','philosoph','buddhism','jainism','folk','tribal','stereotype','offline','devotional']) assert.ok(allText.includes(required),`missing Chapter 7 school-needs boundary: ${required}`);
...
for(const scenario of validation.scenarios){
  ...
  assert.ok(!/ask .*learner.*(?:religion|caste|tribe|sect|community|belief)/.test(text),`${scenario.id} requests sensitive identity disclosure`);
...
console.log(`CLASS6_SOCIAL_SCIENCE_CH7_SCHOOL_NEEDS_PASS scenarios=${validation.scenarios.length} topics=${topicCoverage.size} realChildData=${validation.privacyBoundary.realChildData}`);
