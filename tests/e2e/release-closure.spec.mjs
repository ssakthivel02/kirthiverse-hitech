...
const CLASS6_SCIENCE_ASSESSMENT_IDS = Array.from({length:120},(_,i)=>`KV-CBSE6-SCI-${String(i+1).padStart(4,'0')}`);
...
test('ACTIVE MASTER release surface preserves canonical corpus and local-first identity boundaries', async ({ page }) => {
  ...
  expect(corpus.scienceLessons).toBe(24);
  expect(corpus.mathTopicIds).toHaveLength(3);
  expect(corpus.scienceTopicIds).toHaveLength(24);
  expect(corpus.lessons).toBe(162);
  expect(corpus.coreAssessments).toBe(72);
  expect(corpus.mathAssessments).toBe(15);
  expect(corpus.scienceAssessments).toBe(120);
  expect(corpus.assessments).toBe(207);
